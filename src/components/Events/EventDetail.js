...
export default function EventDetail({ match, handleLogout, allEvents }) {
  ...
  const geoCoder = (address) => {
    ...
    axios
      ...
      // )
      .get(
        `${process.env.REACT_APP_MAP_URL}/geocoding/v5/mapbox.places/${street}%20${number}.json?access_token=${process.env.REACT_APP_MAPBOX_TOKEN}&cachebuster=1609931617105&autocomplete=true`
      )
      .then((data) => {
        setViewport((prev) => ({
          ...prev,
